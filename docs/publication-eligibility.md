# Publication eligibility

- Repository: `chouyong/dsh-effect-doctor`
- URL: `https://github.com/chouyong/dsh-effect-doctor`
- Visibility: `public`
- GitHub `created_at`: `2026-08-16T11:35:15Z`
- Earliest 24-hour eligibility: `2026-08-17T11:35:15Z`
- Product commit count after final review closeout: `17`
- Required product commit count: at least `10` real functional commits
- Stage 0 decision: `GO_ENGINE`
- Stage 1–3 runtime evidence: passed at final cleanup-precedence source commit `b638cdd55e1fdb1c39e9b1f8eaab3070d737f55d`; Stage 3 exact hashes and checks are in `docs/STAGE_3_DSH_GATE.md`
- Claude final technical review: `GO`; R5 aggregate receipt SHA-256 `fe4c3003e181c4ffafded368a5c0b43e41844780a1f3c1c23ab5bd73d7b0dcf1`
- Awesome-list PR status: `WAITING_ELIGIBILITY`

At the final pre-publication check, local UTC was `2026-08-16T14:22:43Z`, only about 2 hours 47 minutes after repository creation and about 21 hours 12 minutes before the earliest eligible instant. The 24-hour timestamp is necessary but not sufficient: a PR remains forbidden until the real GitHub clock reaches it, and target-list rules plus the focused PR diff pass afterwards.
